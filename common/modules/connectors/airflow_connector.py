...
class AirflowConnector(Connector):
    ...
    def post_dag_run(self, dag_name, dag_uuid, dag_config):
        ...
        :param str dag_name: Name of the airflow DAG to run, aka dag_id
    ...
    def get_dag_run_state(self, dag_name, dag_uuid):
        """
        Get a existing DAG run state from the airflow instance on the ml-backend

        :param str dag_name: Name of the airflow DAG to run, aka dag_id
        :param array dag_uuid: UUID for the DAG run

        :return: "queued", "running", "success", "failed", "error"
        """
        try:
            api_instance = dag_run_api.DAGRunApi(self.client)
            # Get a DAG run
            api_response = api_instance.get_dag_run(dag_id=dag_name, dag_run_id=str(dag_uuid))
            self.logger.info(api_response)
            # print(api_response)
            # print(api_response.state)
            return str(api_response.state).lower()
        except (airflow_api.ApiException, RequestError, ConnectionError, NewConnectionError) as err:
            self.logger.error(err)
            return "error"
