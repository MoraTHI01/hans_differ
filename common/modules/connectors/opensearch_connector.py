...
DOCUMENTS_VECTOR_INDEX_NAME = os.getenv("HANS_BACKEND_OPENSEARCH_DOCUMENTS_VECTOR_INDEX")
...
class OpensearchConnector(Connector):
    ...
    def __init__(self, host, port, user, password, vector_size):
        """
        Initialization
        ...
        :param int vector_size: opensearch vector size
        """
        self.client = None
        self.host = host
        self.port = port
        ...
        self.documents_vector_index = DOCUMENTS_VECTOR_INDEX_NAME
    ...
    def insert_lecture_vector_entry(
        self, lecture_id: str, chunk_id: Optional[str], json_data: Dict[str, Any], index: str = VECTOR_INDEX_NAME
    ):
        if self.client is None:
            self.connect()
        try:
    ...
    def vector_search_in_specific_lectures(
        ...
        try:
            ...
            results = self.create_result_list_from_lecture_vector_search_response(
                response, width_data=width_data, index=index
            )
            return results
        except opensearchpy.exceptions.OpenSearchException as err:
    ...
    def insert_course_document_vector_entry(
        self,
        course_id: str,
        document_id: Optional[str],
        json_data: Dict[str, Any],
        index: str = DOCUMENTS_VECTOR_INDEX_NAME,
    ):
        if self.client is None:
            self.connect()
        try:
            ...
            response = self.client.index(index=index, body=json_data, refresh=True, id=entry_id, request_timeout=120)
            self.logger.log(logging.INFO, f"New course vector entry in index '{index}' - id: '{response['_id']}'")
            return True
        except opensearchpy.exceptions.OpenSearchException as err:
            self.logger.log(
                logging.INFO, f"Cannot insert course vector entry with chunk_id '{chunk_id}' to index '{index}'"
            )
            self.logger.error(err)
            return False

    def delete_all_vector_entries_of_course(self, course_id: str) -> bool:
        """
        Delete all vector entries belonging to specific course.

        :param str course_id: metadata uuid of lecture
        ...
        try:
            response = self.client.delete_by_query(
                index=DOCUMENTS_VECTOR_INDEX_NAME, body={"query": {"term": {"course_id": course_id}}}
            )
            if "deleted" in response:
                self.logger.log(
                    logging.INFO,
                    f"Successfully deleted {response['deleted']} entries with "
                    f"course_id '{course_id}' from the index '{DOCUMENTS_VECTOR_INDEX_NAME}'.",
                )
                return True
            else:
                self.logger.log(
                    logging.ERROR,
                    f"Failed to delete entries with course_id '{course_id}' "
                    f"from the index '{DOCUMENTS_VECTOR_INDEX_NAME}'.",
                )
                return False
        except opensearchpy.exceptions.OpenSearchException as err:
            self.logger.log(
                logging.INFO,
                f"Cannot delete entries with document id '{course_id}' "
                f"from the index '{DOCUMENTS_VECTOR_INDEX_NAME}'.",
            )
            self.logger.error(err)
            return False

    def delete_vector_entries_for_document(self, document_id: str) -> bool:
        """
        Delete document vector entry of a document.

        :param str document_id: metadata uuid of document
        ...
        try:
            response = self.client.delete_by_query(
                index=DOCUMENTS_VECTOR_INDEX_NAME, body={"query": {"term": {"document_id": document_id}}}
            )
            if "deleted" in response:
                self.logger.log(
                    logging.INFO,
                    f"Successfully deleted {response['deleted']} entry with "
                    f"document_id '{document_id}' from the index '{DOCUMENTS_VECTOR_INDEX_NAME}'.",
                )
                return True
            else:
                self.logger.log(
                    logging.ERROR,
                    f"Failed to delete entry with document_id '{document_id}' "
                    f"from the index '{DOCUMENTS_VECTOR_INDEX_NAME}'.",
                )
                return False
        except opensearchpy.exceptions.OpenSearchException as err:
            self.logger.log(
                logging.INFO,
                f"Cannot delete entry with document id '{course_id}' "
                f"from the index '{DOCUMENTS_VECTOR_INDEX_NAME}'.",
            )
            self.logger.error(err)
            return False

    def vector_search_in_specific_documents(
        self,
        query_vector,
        ...
        """
        Perform vector search on all chunk entries belonging to provided document_ids.

        :param query_vector: vector to use for similarity search
        :param document_ids: IDs of all documents to include in search, can be list of one ID, usually part of one course with one course id
        :param num_results: number of similar chunks to return
        :param width_data: return data field of vector image
        ...
        try:
            knn_query = {
                ...
                "query": {
                    "script_score": {
                        "query": {
                            "terms": {"document_id": document_ids}  # Use 'terms' to match any document_id from the list
                        },
                        "script": {
                            "source": "knn_score",
                            "lang": "knn",
            ...
            response = self.client.search(index=index, body=knn_query)
            results = self.create_result_list_from_document_vector_search_response(response, width_data=width_data)
            return results
        except opensearchpy.exceptions.OpenSearchException as err:
            self.logger.log(logging.INFO, f"Search document vector index - not successful")
            self.logger.error(err)
            return None

    @staticmethod
    def create_result_list_from_document_vector_search_response(response, width_data=False) -> List[Dict[str, Any]]:
        result_list = []
        for hit in response["hits"]["hits"]:
            result = dict()
            result["text"] = hit["_source"]["text_segment"]
            result["document_id"] = hit["_source"]["document_id"]
            result["segment_id"] = hit["_source"]["segment_id"]
            result["segment_index"] = hit["_source"]["segment_index"]
            result["pages"] = hit["_source"]["pages"]
    ...
    @staticmethod
    def create_result_list_from_lecture_vector_search_response(
        response, width_data=False, index=VECTOR_INDEX_NAME
    ) -> List[Dict[str, Any]]:
        result_list = []
        if index == VECTOR_INDEX_NAME or index == SLIDES_VECTOR_INDEX_NAME:
            for hit in response["hits"]["hits"]:
                result = dict()
                ...
                if width_data is True:
                    result["data"] = hit["_source"]["data"]
                result_list.append(result)
        elif index == SUMMARIES_VECTOR_INDEX_NAME:
            for hit in response["hits"]["hits"]:
                result = dict()
                result["lecture_id"] = hit["_source"]["lecture_id"]
                result["score"] = hit["_score"]
